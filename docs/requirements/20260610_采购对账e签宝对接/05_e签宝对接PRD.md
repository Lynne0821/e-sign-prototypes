...
| 2026-06-10 | 基于会议纪要更新：1. 过滤样品采购订单；2. 签署中遭遇金蝶反审核自动强制撤回；3. 固定我方签署人及无序签；4. 增加携客云状态回写；5. 明确数据查询权限及特权账号。 | 业务产品/系统分析 | v1.2 |
...
- **处理规则**：
  - **状态与异常监控**：拉取金蝶“采购单”和携客云“对账单”。其中金蝶采购单需增加过滤条件：`单据类型 != 样品采购订单`。只有当上游状态为`已审核`（金蝶）或`已确认`（携客云）时才属于正常状态。若拉取到其他状态（如反审核、取消确认），交付系统需在列表中通过独立的异常标识（如`上游异常`）进行高亮警示拦截。
...
- **主流程步骤**：
  ...
  2. 系统按序调用 e签宝接口：自动上传 PDF 文件 -> 创建签署任务。**注意：** 内部签署方信息固定传入 `千岩，王海飞(15814008680)` 并指定加盖 `采购专用章`；任务发起设定为**无序签**（建议业务后用印）；同时包含供应商主体及对应签署人手机号等坐标信息。
  3. 交付系统本地将状态自动置为`签署中`。
...
- **异常情况与处理**：
  ...
  - **上游状态突变拦截**：若单据处于“待发起”且上游变为非审核状态，禁止发起；若处于“签署中”且上游状态变为异常（如金蝶反审核），系统**自动调用 e签宝接口强制作废该签署任务**，单据状态变更为“已作废”；若单据已签署完成，若业务需要改单必须线下关闭订单重走，**系统不自动更新**，仅作异常高亮展示。
  - **手动辅助下发**：提供“复制签署链接”能力，便于采购/财务通过微信等私域工具将 e签宝短链定向发给供应商签署人。

#### 功能点 4：单据完成状态回写（下游回流）
- **业务目标**：形成线上签署状态在业务流的闭环，辅助后续付款与存档。
- **处理规则**：
  - 交付系统在接收到 e签宝的“签署完成” Webhook 并且归档本地生效 PDF 后，自动调用携客云对应的状态更新接口。
  - 将对应采购单、对账单在携客云的【签章状态】变更为【完成】。
...
| 角色 | 数据范围 | 单据状态 | 可执行操作 |
| --- | --- | --- | --- |
| 全局特权(如: 王海飞) | 全量采购单与对账单 | 所有状态 | 全部权限 |
| 采购员 | 采购员 = 当前登录人的采购单 | 待发起 (异常) | 去补充配置、跳转上游详情 |
| 采购员 | 采购员 = 当前登录人的采购单 | 签署中 / 拒签 | 复制签署链接、同步状态、重新发起签署、强制撤回 |
| 财务/结算员 | 审核人 = 当前登录人的对账单 | 待发起 (异常) | 去补充配置、跳转上游详情 |
| 财务/结算员 | 审核人 = 当前登录人的对账单 | 签署中 / 拒签 | 复制签署链接、同步状态、重新发起签署、强制撤回 |
...
| 编号 | 待决问题 | 责任方 | 目标结论日期 |
| --- | --- | --- | --- |
| 1 | **合同条款声明变更**：新生成的 PDF 中，底部的说明文字需要业务部门和法务确认从“传真/邮件扫描视为有效”统一更改为电子签章具备法律效力的标准化表述，并提供最终文本。 | 业务 / 法务 | 研发提测前 |
| 2 | **企业认证流程**：供应商在收到 e签宝通知时，如果其主体之前从未在 e签宝注册和实名，e签宝的认证前置流程对某些小微供应商是否有阻力，需确定沟通推进话术。 | 采购运营 | 上线前 |
